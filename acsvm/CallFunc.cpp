...
namespace ACSVM
{
   ...
   bool CallFunc_Func_ScrStartS(Thread *thread, Word const *argv, Word argc)
   {
      ...
      {
         res = true;
         thread->scopeMap->scriptStart(script, {argv+2, argc-2, thread->getInfo()});
      }
      else
         res = false;
   ...
   bool CallFunc_Func_ScrStartSF(Thread *thread, Word const *argv, Word argc)
   {
      ...
      {
         res = true;
         thread->scopeMap->scriptStartForced(script, {argv+2, argc-2, thread->getInfo()});
      }
      else
         res = false;
   ...
   bool CallFunc_Func_ScrStartSR(Thread *thread, Word const *argv, Word argc)
   {
      ...
      if(Script *script = thread->scopeMap->findScript(name))
         res = thread->scopeMap->scriptStartResult(script, {argv+1, argc-1, thread->getInfo()});
      else
         res = 0;
